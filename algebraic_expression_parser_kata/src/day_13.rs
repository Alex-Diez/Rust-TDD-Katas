...
impl<'p> Parser<'p> {
    ...
    pub fn parse_expression<'s>(&mut self) -> Ast {
        let mut root = self.parse_term();
        while let Some(operand) = self.low_priority_operation() {
            root = Ast::Operation(operand, Box::new(root), Box::new(self.parse_term()))
        }
        root
    }

    fn low_priority_operation(&mut self) -> Option<char> {
...
mod tests {
    ...
    fn parse_subtraction() {
        let expr = Cow::Borrowed("4-1");
        assert_eq!(Parser::new(&expr).parse_expression(), operation('-', num(4.0), num(1.0)));
    }

    #[test]
    fn parse_multiplication() {
        let expr = Cow::Borrowed("5×2");
        assert_eq!(Parser::new(&expr).parse_expression(), operation('×', num(5.0), num(2.0)));
    }

    #[test]
    fn parse_division() {
        let expr = Cow::Borrowed("10÷2");
        assert_eq!(Parser::new(&expr).parse_expression(), operation('÷', num(10.0), num(2.0)));
    }

    #[test]
    fn parse_many_operations() {
        let expr = Cow::Borrowed("1-2÷3+2×3÷1");
        assert_eq!(
    ...
    fn parse_expression_with_parenthesis() {
        let expr = Cow::Borrowed("(1-2)÷(3+2)×3");
        assert_eq!(
            Parser::new(&expr).parse_expression(),
            operation(
                '×',
