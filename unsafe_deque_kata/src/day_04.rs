...
impl <'d, T> Iterator for UnsafeDequeIterRef<'d, T> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        self.node.take().map(|node| unsafe {
            self.node = node.next.as_ref().map(|node| &*node.as_ref());
            &node.item
        })
    }
}
...
impl <'d, T> Iterator for UnsafeDequeIterRefMut<'d, T> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        self.node.take().map(|node| unsafe {
            self.node = node.next.as_mut().map(|node| &mut *node.as_mut() );
            &mut node.item
        })
    }
}
...
mod tests {
    ...
    fn peek_back_mut() {
        ...
        assert_eq!(deque.peek_back(), Some(&6));
    }
