...
impl<T> UnsafeDeque<T> {
    ...
    pub fn push_front(&mut self, item: T) {
        let boxed_head = Node::new(item);
        unsafe {
            let new_head = Shared::new(Box::into_raw(boxed_head));
    ...
    pub fn push_back(&mut self, item: T) {
        let boxed_tail = Node::new(item);
        unsafe {
            let new_tail = Shared::new(Box::into_raw(boxed_tail));
...
impl<'mi, T> IntoIterator for &'mi mut UnsafeDeque<T> {
    ...
    fn into_iter(self) -> Self::IntoIter {
        RefMutDequeIterator { head: self.head, tail: self.tail, empty: false, data: PhantomData }
    }
}
...
impl<'i, T> IntoIterator for &'i UnsafeDeque<T> {
    ...
    fn into_iter(self) -> Self::IntoIter {
        RefDequeIterator { head: self.head, tail: self.tail, empty: false, data: PhantomData }
    }
}
...
mod tests {
    ...
    fn pushes_many_in_front(deque: &mut UnsafeDeque<i32>, times: i32) {
