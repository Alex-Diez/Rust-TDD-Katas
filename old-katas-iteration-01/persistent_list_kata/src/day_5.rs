...
mod tests {
    ...
    #[test]
    fn drop_one_item_from_a_list() {
        assert_eq!(List::from(vec![1, 2, 3]).drop(1), List::from(vec![1, 2]));
    }
    ...
    #[test]
    fn drop_all_items_by_a_predicate() {
        assert_eq!(List::from(vec![1, 2, 3]).drop_while(|item| item > 0), List::empty());
    }
    ...
    #[test]
    fn takes_one_item_from_a_list() {
        assert_eq!(List::from(vec![1, 2, 3]).take(1), List::single(3));
    }
