...
pub trait MergeSort<T: Ord> {
    ...
    fn merge(data: *mut T, auxilary: *mut T, low: isize, middle: isize, high: isize) {
        let mut i = low;
        let mut j = middle + 1;
        unsafe {
...
impl <T: Ord> MergeSort<T> for TopDownMergeSort { }
...
impl <T: Ord> Sort<T> for TopDownMergeSort {
    ...
    fn sort(data: &mut [T]) {
        ...
        TopDownMergeSort::sort_inner(data_ptr, auxilary_ptr, 0, (len - 1) as isize);
    }
}

pub struct BottomUpMergeSort;

impl <T: Ord> MergeSort<T> for BottomUpMergeSort { }
...
impl QuickSort {
    ...
    fn partition<T: Ord>(data: *mut T, low: isize, high: isize) -> isize {
        ...
        unsafe {
            ...
            loop {
                while &*data.offset(i) < &*value {
                    if i == high {
                        break;
                    }
                    i += 1;
                }
                while &*data.offset(j) > &*value {
                    if j == low {
                        break;
                    }
                    j -= 1;
