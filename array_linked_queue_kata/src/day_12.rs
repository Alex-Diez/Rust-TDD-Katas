...
impl <T: Default + Copy + PartialEq> Segment<T> {
    fn new() -> SegmentLink<T> {
        Rc::new(RefCell::new(Segment { head: 0, tail: 0, items: [T::default(); SEGMENT_SIZE], next: None }))
    }
...
mod tests {
    ...
    #[test]
    fn enqueue_deque_more_than_segment() {
        let mut queue = ArrayLinkedQueue::default();
    ...
    #[test]
    fn enqueue_deque_one_by_one_more_than_segment() {
        let mut queue = ArrayLinkedQueue::default();
