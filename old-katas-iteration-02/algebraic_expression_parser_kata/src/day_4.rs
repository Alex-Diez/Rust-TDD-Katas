...
mod tests {
    ...
    fn parse_negative_number() {
        assert_eq!(parse_expression(Cow::Borrowed("-5")), num(-5.0));
    }

    #[test]
    fn parse_addition() {
        assert_eq!(parse_expression(Cow::Borrowed("4+3")), operand('+', num(4.0), num(3.0)));
    }

    #[test]
    fn parse_subtraction() {
        assert_eq!(parse_expression(Cow::Borrowed("3-2")), operand('-', num(3.0), num(2.0)));
    }
    ...
    fn parse_many_operations() {
        assert_eq!(
            parse_expression(Cow::Borrowed("3-5÷1+2×7")),
            operand(
                '+',
                operand(
                    '-',
    ...
    fn parse_expression_with_parenthesis() {
        assert_eq!(
            parse_expression(Cow::Borrowed("((3-5)÷1+2)×7-1")),
            operand(
                '-',
                operand(
                    '×',
