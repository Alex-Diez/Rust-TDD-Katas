...
impl ToString for List {
    fn to_string(&self) -> String {
        fn internal(list: &List) -> String {
            match list {
                &List::Empty => "".to_owned(),
                &List::Cons(head, ref tail) => ", ".to_owned() + head.to_string().as_str() + internal(&*tail).as_str()
            }
        }
        match self {
            &List::Empty => "[]".to_owned(),
...
mod tests {
    ...
    fn concatenation_of_empty_and_nonempty_lists() {
        ...
        assert_eq!(empty + (List::empty() + 1 + 2 + 3), List::empty() + 1 + 2 + 3);
