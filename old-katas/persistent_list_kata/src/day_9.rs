...
mod tests {
    ...
    #[test]
    fn reverses_a_list() {
        assert_eq!(List::from(vec![1, 2, 3, 4, 5]).reverse(), List::from(vec![5, 4, 3, 2, 1]));
    }

    #[test]
    fn takes_from_a_list() {
        assert_eq!(List::from(vec![1, 2, 3, 4, 5]).take(3), List::from(vec![3, 4, 5]));
    }

    #[test]
    fn takes_from_a_list_by_predicate() {
        assert_eq!(List::from(vec![1, 2, 3, 4, 5]).take_while(|item| item > 3), List::from(vec![4, 5]));
    }

    #[test]
    fn maps_over_a_list() {
        assert_eq!(List::from(vec![1, 2, 3, 4, 5]).map(|item| item * 2), List::from(vec![2, 4, 6, 8, 10]));
    }
}