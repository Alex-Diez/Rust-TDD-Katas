...
pub struct UnidirectGraph {
    vertices: HashMap<isize, Vec<isize>>,
    edges: usize
}

impl Default for UnidirectGraph {

    fn default() -> UnidirectGraph {
        UnidirectGraph {
...
impl DepthFirstSearch {
    ...
    pub fn new(graph: &UnidirectGraph, src: isize) -> Option<DepthFirstSearch> {
        if graph.vertices() > 1 {
    ...
    fn search(&mut self, graph: &UnidirectGraph, v: isize) {
...
impl BreadthFirstSearch {
    ...
    pub fn new(graph: &UnidirectGraph, src: isize) -> Option<BreadthFirstSearch> {
        if graph.vertices() > 1 {
    ...
    fn search(&mut self, graph: &UnidirectGraph, s: isize) {
