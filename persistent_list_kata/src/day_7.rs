...
mod tests {
    ...
    fn drops_from_a_list_by_predicate() {
        assert_eq!(List::from(vec![1, 2, 3, 4]).drop_while(|item| item > 2), List::from(vec![1, 2]));
    }
    ...
    #[test]
    fn takes_items_from_a_list() {
        assert_eq!(List::from(vec![1, 2, 3, 4]).take(2), List::from(vec![3, 4]));
    }

    #[test]
    fn takes_items_from_a_list_by_predicate() {
        assert_eq!(List::from(vec![1, 2, 3, 4]).take_while(|item| item > 3), List::single(4));
    }

    #[test]
    fn maps_over_a_list() {
        assert_eq!(List::from(vec![1, 2, 3, 4]).map(|item| item * item), List::from(vec![1, 4, 9, 16]));
    }
}