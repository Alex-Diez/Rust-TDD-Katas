...
impl <T: ToString> ToString for List<T> {
    fn to_string(&self) -> String {
        ...
        match self {
            &List::Empty => "[]".to_string(),
            &List::Cons(ref head, ref tail) => "[".to_string() + head.to_string().as_str() + internal(&*tail).as_str() + "]"
        }
    }
}
...
mod tests {
    ...
    fn tail_of_nonempty_list() {
        let list = List::from_iter(vec![1, 2, 3]);
        assert_eq!(list.tail(), List::from_iter(vec![1, 2]));
    }
    ...
    #[test]
    fn concatenation_of_nonempty_list() {
