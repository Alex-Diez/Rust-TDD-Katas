...
impl <T: ToString> ToString for List<T> {
    fn to_string(&self) -> String {
        fn from_node<E: ToString>(head: &E, tail: &Box<List<E>>) -> String {
        ...
        fn internal<E: ToString>(list: &List<E>) -> String {
            match list {
                List::Empty => "".to_owned(),
                List::Cons(head, tail) => ", ".to_owned() + from_node(head, tail).as_str()
            }
        }
...
mod tests {
    ...
    fn tail_of_nonempty_list() {
        ...
        assert_eq!(list.tail(), Ok(List::empty() + 1 + 2));
    }
